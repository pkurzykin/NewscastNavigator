...
const older = { ...first, id: 93, started_at: "2026-07-11T08:00:00Z", ended_at: "2026-07-11T08:05:00Z", from_revision: 8, to_revision: 9, diff_href: "/api/v1/stories/101/history/edit-sessions/93", available_actions: [] };
...
async function installHistoryApi(page: Page): Promise<void> {
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (path === older.diff_href && request.method() === "GET") return route.fulfill({ json: { story, session: older, changes: [{ segment_uid: "seg_older", kind: "changed", moved: false, changed_fields: ["text"], before: { order_index: 1, block_type: "zk", text: "Старая адресная редакция" }, after: { order_index: 1, block_type: "zk", text: "Нужная адресная редакция" } }] } });
...
test("direct history session URL expands an older diff and survives reload", async ({ page }) => {
  await installHistoryApi(page);
  await page.goto("/stories/101/history?session=93");

  await expect(page).toHaveURL(/\/stories\/101\/history\?session=93$/);
