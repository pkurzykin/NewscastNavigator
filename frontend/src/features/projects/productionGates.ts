...
function isTrackDone(status?: string | null): boolean {
  return (status || "").trim().toLowerCase() === "done";
}

function isTrackInProgress(status?: string | null): boolean {
...
function isFinalApprovalDone(status?: string | null): boolean {
  return (status || "").trim().toLowerCase() === "approved";
}

function isFinalApprovalActive(status?: string | null): boolean {
  const normalized = (status || "").trim().toLowerCase();
...
export function buildProductionGates(project: ProjectListItem): ProductionGate[] {
  ...
  const textNeedsAttention = Boolean(
    project.current_text_seq &&
      (!project.current_text_is_latest || !project.latest_text_is_proofread)
  );

  const voiceoverReady = Boolean(
    project.voiceover_text_seq &&
      isTrackDone(project.voiceover_status) &&
      !project.voiceover_requires_resync
  );
  const voiceoverNeedsAttention = Boolean(
    project.voiceover_requires_resync || isTrackInTrouble(project.voiceover_status)
  );

  const editReady = Boolean(
    project.edit_text_seq &&
      isTrackDone(project.edit_status) &&
      !project.edit_requires_resync
  );
  const editNeedsAttention = Boolean(
    project.edit_requires_resync || isTrackInTrouble(project.edit_status)
  );
  ...
  const titlesNeedAttention = Boolean(
    project.titles_requires_resync || isTrackInTrouble(project.titles_status)
  );
  ...
  const gates: ProductionGate[] = [
    {
      key: "text_ready",
      label: "Текст готов",
      status: textReady ? "done" : textNeedsAttention ? "attention" : "current",
      summary: textReady ? "Текущий и вычитанный" : "Нужен готовый текст",
      detail: `Текущий ${seqLabel(project.current_text_seq)} · вычитанный ${seqLabel(project.proofread_text_seq)}`,
      actionLabel: textReady ? "Текст можно отдавать в производство" : "Подготовить текущий вычитанный текст",
    },
    {
      key: "voiceover_ready",
      label: "Озвучка готова",
      status: voiceoverReady
        ? "done"
        : voiceoverNeedsAttention
          ? "attention"
          : textReady
            ? "current"
            : "blocked",
      summary: voiceoverReady ? "Готовая озвучка" : trackStatusLabel(project.voiceover_status),
      detail: `Озвучка ${seqLabel(project.voiceover_text_seq)} · вычитка ${seqLabel(project.proofread_text_seq)}`,
      actionLabel: voiceoverReady ? "Озвучка актуальна" : "Получить или обновить озвучку",
    },
    {
      key: "edit_review",
      label: "Проверка монтажа",
      status: editReady
        ? "done"
        : editNeedsAttention
          ? "attention"
          : voiceoverReady
            ? "current"
            : "blocked",
      summary: editReady ? "Монтаж OK" : trackStatusLabel(project.edit_status),
      detail: `Монтаж ${seqLabel(project.edit_text_seq)} · текущий текст ${seqLabel(project.current_text_seq)}`,
      actionLabel: editReady ? "Монтаж принят" : "Монтаж OK или зафиксировать правки",
    },
    {
      key: "titles",
      label: "Титры",
      ...
      status: titlesDone
        ...
        : editReady && titlesNeedAttention
          ...
          : titlesReadyForReview
            ? "current"
            : "blocked",
      summary: titlesDone ? "Титры OK" : trackStatusLabel(project.titles_status),
...
export function getCurrentProductionGate(project: ProjectListItem): ProductionGate {
  ...
  return (
    gates.find((gate) => gate.status === "attention") ||
    gates.find((gate) => gate.status === "current") ||
