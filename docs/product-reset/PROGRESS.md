...
## Продолжение: выбран trial 14 сентября и очистка всего frontend

Пользователь заменил прежний визуальный эталон таблицы на вариант «3 с минусом»
и поручил исключить смешение старого и нового оформления. Работа ведётся в той
же ветке. Завершено на `08f25bb`: тонкая сетка, прямоугольные controls,
IN/OUT и две кнопки основных шрифтов. Объединены 13 повторных правил редактора,
проверены все 11 CSS, тема и компоненты. Устранены старый синий button default,
хвостовые overrides и лишняя внешняя карточка Истории. Полный frontend: 558 passed;
browser: 151 passed / 2 прежних BFCache skips; build и независимое ревью — PASS.
Стенд пересобран и проверен. [Итог очистки](UI_SYSTEM_STYLE_CLEANUP_RU.md).
Порог квоты — 3%.
Итог предыдущего checkpoint ниже относится к прежнему выбору макета.

## Визуальная доводка всех экранов и принятой таблицы — 15 сентября

- Пользователь подтвердил завершение визуальной реализации, включая таблицу 6/13 сентября.
- Исправлена специфичность общих CSS defaults; возвращены компонентные размеры, пастельные типы блоков и круглая палитра.
- Убраны повтор заголовка и лишняя внешняя карточка редактора. Workflow/actions/access/font/formatting собраны по принятой компоновке.
- Несценарные экраны уплотнены; технические подписи удалены. Функции и шрифты текста сохранены.
- Подробности, проверки и локальный preview: [UI_SYSTEM_VISUAL_POLISH_RU.md](UI_SYSTEM_VISUAL_POLISH_RU.md).

## Дизайн-система — продолжение с пользователем 15 сентября

- Поднят отдельный локальный стенд `ncn-ui-system-preview` из текущей ветки.
- Пройден базовый browser → FastAPI → PostgreSQL smoke: создание, четыре блока,
  autosave/шрифт после reload, вторая вкладка, diff/collapse/restore, назначения.
- В `d0e3423` исправлена найденная подпись восстановления: технические номера
  больше не показываются в обычной ленте. 13 API-тестов Истории прошли после RED.
- После исправления полный backend: 1072 passed / 4 прежних PostgreSQL-only skips /
  2209 унаследованных warnings. Новая подпись проверена в работающем браузере.
- Стенд оставлен для приёмки; Colima-настройки и основной checkout не менялись.
  Подробности: [UI_SYSTEM_LIVE_CHECK_RU.md](UI_SYSTEM_LIVE_CHECK_RU.md).
- Ниже сохранён отчёт исходного пакета `f1ed4cf`; его полные результаты относятся
  к указанному SHA, а не автоматически к последующим дополнениям.
...
- `f103f99`: пустой/null сохранённый per-field font теперь наследует основной,
  не меняя сохранённые строки, ручной PT Sans или оформление. Characterization
  mock дополнен действительным контрактом `getAttributes`; runtime не ослаблен.
- Полный frontend: 557 passed / 59 files, без unhandled errors; build passed.
  Направленные проверки: 49 frontend и 1 backend. Последняя backend-проверка
  одновременно меняет revision, rows и Franklin перед lock и проверяет цельный GET.
- Task5 scoped re-review Approved, оба P2 закрыты. Task7c `0e9aac0` также Approved:
  меню профиля имеет доступное имя, browser проверяет ровно 56 ± 1 px.
- Полный backend `8934c35`: 1071 passed / 1 устаревшее ожидание / 4 skipped;
  это ожидание исправлено в `f103f99`. Итоговый полный прогон ещё предстоит.

## Дизайн-система — шрифт сценария и История (на ревью)

- Основной шрифт PT Sans / Franklin Gothic Book входит в единый snapshot вместе
  со строками: autosave, undo/redo, локальный черновик, idempotency, история,
  восстановление и DOCX. Ручные шрифты и оформление сохраняются. Миграция
  `20260914_0005` добавляет настройку без переписывания строк.
- В Истории добавлены подсветка изменённых слов, признаки удаления/добавления,
  описание изменений оформления и основного шрифта. Сравнение сворачивается
  даже во время загрузки; поздний ответ его не раскрывает.
- Восстановление выбирает состояние на конец указанного сеанса и сохраняет
  последующую историю. Уже актуальное состояние сообщает об отсутствии изменений;
  после подтверждённой команды при ошибке обновления повторяется только GET.
- Удалены прежняя связь «кэш = открытая панель», глобальная блокировка загрузки
  одного сравнения и материализация наследуемого PT Sans при выборе B/I.
- Scoped backend 101 passed / 1 PostgreSQL-only skip, frontend 302 passed;
  PostgreSQL 38 passed, включая migration/autosave/archive races. Дополнительные
  тесты frozen DOCX snapshot и font-only notifications — 2 passed.
- Browser регрессии редактора/доступа/экспорта: 72 passed / 2 BFCache skips на
  1366 и 1920. История: component 47 passed; browser 12 passed и 2 ошибки ожидания
  фокуса в тесте, исправленный узкий повтор — 2 passed. Полный общий прогон впереди.
- Canonical DOCX двух основных гарнитур отрисован bundled LibreOffice; проверены
  все 6 страниц, PDF реально содержит PT Sans / Franklin Gothic Book. Временный
  FONTCONFIG_FILE не меняет Mac. Отдельный ручной Roboto Slab отсутствует у
  рендерера и подменяется; OOXML сохраняет его имя. Windows Word ещё не проверялся.
- Downgrade новой миграции удаляет необратимый font-aware retry hash cache;
  compact retries без сохранённых rows после отката ограничены. Forward upgrade
  сохраняет прежние compact hashes оборачиванием; текстовые snapshots не теряются.
- Независимое ревью, итоговый полный прогон и локальный clean-deploy rehearsal
  ещё не завершены; перечисленное не является финальным допуском к публикации.

## Дизайн-система — промежуточная интеграционная проверка

- На неизменяемом `069a3ed`: frontend 514 passed / 3 failed; backend 1063 passed /
  1 failed / 4 skipped; browser 1366 — 57 passed / 2 failed / 1 skipped.
  Это промежуточный прогон, не итоговый успешный gate.
- Frontend failures — отсутствующий новый GET автора в двух фикстурах и прежняя
  подпись ссылки на историю. Исправленные проверки: 21 passed. Backend failure —
  исторические CP1-классификации inventory; текущий список дополнен пятью policy/
  runbook путями, проверяются существование текущих файлов и отсутствие legacy.
  Повтор repository/current docs: 13 passed.
- Два browser failures относятся к первому входу в редактор и жёстко заданному
  порту второй вкладки; исправляются и проверяются в checkpoint 3.
...
## Дизайн-система — checkpoint 4: клавиши и GEO

- `e842668`: NumpadSubtract без модификаторов и Alt/Option+Minus вставляют
  среднее тире U+2013; дефис/underscore сохраняются. Физическая Quote даёт
  прямую кавычку только при фактическом вводе двойной кавычки; русский Shift2
  сохраняет «ёлочки». GEO по умолчанию жирный курсив; явные overrides сохранены.
- Frontend 474 passed; DOCX renderer 25 passed; browser typography 2/2 на
  каждой из 1366/1920. Независимое Codex review: замечаний нет.
- Отдельный backend full worker: 1045 passed, 2 skipped, два failures inventory
  в момент добавления MUI зависимостей; после обновления notices focused policy
  2 passed. Второй полный backend не выдаётся за чистый pass.
- Exact CaptionPanels JSON с дефисом, U+2013 и обоими видами кавычек также
  проверен через реальный API. Физические RU/EN-клавиатуры и OS IME остаются
  ручной платформенной проверкой; synthetic key events её не заменяют.

## Дизайн-система — checkpoint 2: список и производство

- В списке статичный автор с fallback username, без должности/переключателя.
  В карточке «Изменить» открывает выбор автора по server management/options;
  chief и chief_editor по-прежнему получают право от backend. После команды
  обновляются только author/management, редактор и его metadata draft не заменяются.
- Исполнители списка: только video_editor и designer в заданном порядке;
  корректор остаётся в Производстве. Поиск исполнителя не отправляет запрос;
  явный выбор сохраняется, «Без исполнителя» снимает назначение. Ошибка оставляет
  выбор и повтор; устаревший callback другой карточки не меняет текущую.
- Фильтры и команды списка объединены, повторный заголовок визуально удалён,
  счётчик только снизу. Производство: компактная полоса этапов, правки/согласование
  слева, исполнители/материалы справа; действия наверху, дополнительные — с
  контекстом этапа. Материал добавляется в диалоге; готовность озвучки сохранена.
- Удалены старый list author-select/callback, внутренний большой Assignments
  с кнопками «Сохранить»/«Снять», отдельная дублирующая VoiceoverState-карточка,
  прежние production grids и заменённые CSS. API identifiers правок сохранены.
- Focused frontend: 48 passed; build pass. Browser первоначально выявил
  устаревшие селекторы и отсутствующую фикстуру нового access endpoint; после
  адаптации production+UI 14 passed. Ранее story-management/axe subset прошёл
  на обеих ширинах. Снимки просмотрены и сохранены в output/implementation.
- Backend correction/CaptionPanels targeted: 27 passed, один label regression
  найден и исправлен, его повтор — 1 passed. Полный immutable checkpoint run
...
### Исправления checkpoint 2 после ревью

- Управление автором доступно также в «Производстве»; ответ защищён от смены
  открытого сюжета. Подтверждённый выбор исполнителя сохраняется до совпадения
  canonical read model; ошибка обновления повторяет только GET.
- В шапке не больше двух команд, остальные размещены у соответствующих этапов.
  Глобальное основное действие отделено семантически от локальных кнопок карточек.
...
## Дизайн-система — checkpoint 6: шаблон и архив (на ревью)

- Новый сюжет получает один раз четыре пустые строки Подводка → ЗК → СНХ → ЗК
  со стабильными segment UID. Просмотр и повторный вход не восстанавливают
  удалённые строки; первый revision boundary сохраняет исходный шаблон.
- DELETE архивного сюжета проверяет свежие функции после блокировки агрегата.
  Любая функция video_editor/designer запрещает удаление, даже вместе с chief.
  Отдельное server delete_action не меняет права восстановления или Production.
- Проверены FK CASCADE всей принадлежащей сюжету структуры; пользователи,
  рубрики, другая карточка и внешний исходник сохраняются. Новый FK не потребовался.
  Архив использует шесть колонок и MUI confirmation с названием, отменой,
  single-flight/ошибкой/повтором; после ack ошибка обновления повторяет только GET.
- TDD: 13 delete + 2 template tests сначала RED; финальный backend subset —
  69 passed / 2 PostgreSQL-only skips. На отдельной локальной PostgreSQL —
  17 passed, включая обе реальные гонки delete/restore с доказанным ожиданием
  row lock. Component — 17 passed; browser — 4 passed на 1366/1920, axe clean.
- Браузер выявил потерю initial focus на контейнер диалога и возврат к удалённой
  кнопке; onEntered и отложенный возврат к заголовку исправлены и проверены.
  Screenshot просмотрен. Полные immutable checks и независимое review впереди;
  реальных данных/материалов и внешних серверов изменения не касались.

### Исправления checkpoint 6 после ревью

- После подтверждения удаления/восстановления строка и счётчик сохраняются до
  успешного canonical GET. Ожидающая строка явно обозначена, её команды недоступны;
...
- Поля входа/пароля и кнопки сотрудников, уведомлений, окна обновления используют
  общую MUI-тему. Native auth validation/autocomplete, функции сотрудников,
  confirmation и жизненный цикл окна обновления сохранены.
- Форма сотрудника компактнее: её команды помещаются на 1366×768. Таблица получила
  общую поверхность; убраны повторный заголовок раздела и прежние стили кнопок.
- Characterization: 36 component tests passed до/после. Начальный browser выявил
  отсутствовавшую fixture scenario/access; новый запрос отражён в тесте. После
  MUI адаптированы selectors обязательных полей со звёздочкой и primary-маркер.
- Browser общих поверхностей: 14 passed / 2 legacy-selector failures; после
  исправления селектора Editorial Air — 4 passed. Последняя проверка изменённого
  admin layout — 6 passed, обе ширины, axe clean; screenshots просмотрены.
  Build pass. Независимое ревью Task7a — Approved; дополнительный существующий
  axe-тест открытых уведомлений/формы сюжета — 2 passed на 1366/1920.
  Окончательный общий прогон ветки остаётся открыт.

## Версия 1.2.0 — инструменты редактора

- Локальный release candidate подготовлен 2026-08-23 в isolated worktree
  `NewscastNavigator-field-corrections-design`, ветка
  `codex/field-corrections-design`. Последовательность checkpoints:
  `c270cf7`, `d9f964d`, `f6acecb` (общая undo/redo history), `f7ff92a`,
...
- TDD version RED: `tests/test_app_version.py` ожидал `1.2.0` при пяти
  ...
  перекрывающий старые fixture-flow; storage contract и чистый release-flow
  разделены тестом. Второй RED выявил устаревший prose locator и очистку всего
...
- Operational inventory: `KEEP` — `compose.yaml`, `compose.test.yaml`,
  ...
  backup/restore/rehearse/smoke/status/update/install/uninstall/scan, а также
  synthetic seed/import/validation/eval/render scripts. `ADAPT`, `REPLACE` и
  `DELETE` — нет: runtime/deploy contract и локальные команды не изменились,
  поэтому `docs/LOCAL_DEV_WORKFLOW_RU.md` оставлен без правок.
- Фактический compiled UI проверен на Mac через repo E2E и bundled Playwright
  CLI на локальном synthetic stack: footer `v1.2.0`; первый показ Whats New с
  точным заголовком, intro, пятью пунктами и initial focus; dismiss и reload
  без повторного окна; home link ведёт на `/stories`. На 1366/1920/2560
  horizontal overflow отсутствует, а header/content left edge совпадает
  (`0/0`, `240/240`, `560/560`). Repo browser-flow также прошёл для `+`/`=`,
  уведомлений без reload, drag и кнопок, undo/redo, кавычек,
  formatted find/replace, atomic Replace All и DOCX flush/export.
- macOS `fc-match` выбрал установленные Franklin Gothic Book Regular и Italic.
  Synthetic DOCX содержит exact family (`152` вхождения в `document.xml`,
  SHA-256 `339e207bcccfe171e866c2bc4333544677234052aa57bea1e733c3e24edce631`);
  bundled LibreOffice открыл его и отрендерил `3` A4-страницы. Все три PNG
  просмотрены: Franklin-текст, шапка, переносы, file bundles, `Гео`, СНХ и
  `Лайф` читаемы; clipping, потерянных границ и пустых страниц нет.
- Новых dependencies, migrations, reverse DOCX import и unintended runtime
...
## 20 сентября 2026 — внедрение принятых форм правок и уведомлений

Пользователь принял два макета и явно разрешил внедрение без дальнейших вызовов
Paper. Работа продолжается в `codex/ui-polish-2026-09-20` из сохранённой
`codex/ui-system` (`7d4c20c`). Постоянный worktree восстановлен в
`/Users/pavelkurzykin/.codex/worktrees/newscast-ui-audit/NewscastNavigator`.
Основной checkout на `main` не изменён. План: `UI_POLISH_2026_09_20_PLAN.md`.

Первый checkpoint: `b8b5745`, уточнение цвета footer `7f410b3`.
Внутренние и внешние правки используют общие поля: область/исполнитель сверху,
описание на всю ширину. Внутренняя форма не показывает лишнюю «Часть 1»;
внешняя сохраняет несколько частей. Оболочка 633 px, отдельная прокрутка тела,
доступные шапка и действия, ошибка возле footer. Удалены старые три колонки
и отдельное правило ширины внешней формы. Поведение API/прав/назначений сохранено.

Проверки первого checkpoint: RED двух новых сценариев, затем 562 frontend-теста
в 59 файлах, TypeScript/Vite build и 2 browser-проверки внутренней формы на
1366/1920 — PASS. Независимое Codex-review: Approved, Critical/Important нет;
minor цвета footer исправлен. CodeRabbit и Paper не использовались.

Интеграционная проверка длинной внешней формы: четыре части, прокрутка тела,
неподвижные действия, ошибка отправки, сохранение всех значений, повторная
отправка и Escape/возврат фокуса на обоих размерах — 2/2 PASS. Исходное падение
нового теста было неверным exact-label локатором; он заменён на combobox по
фактическому доступному имени. Скриншоты проверены визуально и сохранены в
локальном `output/ui-polish-2026-09-20/`.

Полный backend-набор из текущего worktree: 1072 passed, 4 PostgreSQL-only skips,
2209 унаследованных deprecation warnings, 271.90 s. Backend в этом этапе не
менялся. Новый PostgreSQL-only прогон не заявляется. `npm ci`: прежние 13
advisories, обновление зависимостей в этот этап не входит.

Короткие служебные события Истории уплотнены отдельно от сеансов редактирования;
фактический вид проверен на живом synthetic preview. Устаревшие утверждения
SPEC/EVAL/плана об отложенной таблице исправлены по прямому решению пользователя
15 сентября. Каталог дизайн-системы и handoff синхронизированы; дальнейшие
интеграционные результаты следуют ниже после завершения уведомлений.

Уведомления: `4125e33`, полная сводка события сохранена, отдельное явное прочтение,
автор и дата/время из ответа, доступное закрытие, загрузка/ошибка/повтор, семантическое
сравнение и корректное сворачивание. CSS-карточки заменены компактным списком;
очередь показывает «10 действий». Сохранены serial refresh и защита от поздних ответов.
После RED трёх новых сценариев: 19 targeted, 567 frontend в 59 файлах,
TypeScript/Vite и 6 focused browser (включая axe) — PASS.

Первый полный browser-прогон нашёл serious ARIA-ошибку декоративной точки;
она исправлена, как и выявленные сравнением с PNG фон шапки/отступы/ссылки.
Также закреплён переход одного уведомления null→diff без нарушения порядка hooks.
Финальный полный browser на этой реализации: **155 passed, 2 BFCache skips**,
1.0 min, без failed. Проверки редактора, autosave, DOCX, доступа, истории,
архива и маршрутов входят в этот набор. Backend/editor-core/scenario в текущем
этапе не менялись. Docs/repository/eval/UX-evidence: **347 passed**, 348
унаследованных предупреждений. Логи проверки — локально в
`output/ui-polish-2026-09-20/` после завершения checkpoint.

Живой preview проверен через API/PostgreSQL: формы/история/очередь/уведомления,
1366×768 и 1920×1080, понятное удаление пустой строки, сворачивание/Escape,
отсутствие browser console errors. Пять исходных непрочитанных уведомлений
не очищались вручную ради проверки; явное прочтение и его ошибки покрыты
component/browser fixtures. Контейнер frontend пересобран из текущего worktree,
backend и сохранённая синтетическая база не пересоздавались.

Уточнение границы прочтения: новая панель не отправляет read при переходе по ссылке.
При этом сохранено действующее правило SPEC (строки разделов про монтаж и уведомления):
сервер подтверждает уведомления монтажёра/дизайнера после фактического открытия
новой редакции сценария в production context. Общее обещание макета «переход никогда
не прочитывает» не отменяет это утверждённое рабочее поведение; каталог уточнён.

Заключительные исправления: `4d8e2db` ограничивает ширину длинного действия,
сохраняя не менее 150 px контексту на проверенных desktop-размерах.
`273028d` закрывает Important scoped review: при одинаковом кратком описании
реально разного оформления нет ложного «Было/Стало», есть честное пояснение
и переход к полному сравнению. Регрессии цвета заливки и сдвига границы
форматированного фрагмента: RED→GREEN. Последний frontend: **568 passed / 59 files**,
20 targeted, build и focused browser 6/6 — PASS. Повторное независимое ревью:
**Approved**, Critical/Important/Minor нет.

Канонический Compose config проверен с явно синтетическими значениями и
`--env-file /dev/null`; реальные секреты не читались. Backend, scenario и
editor-core не изменены относительно `7d4c20c`. Внешних действий нет;
рабочая ветка и локальный preview сохраняются для дальнейшей приёмки.


Финальное общее ревью нашло ещё два остатка; оба закрыты:
- `8e7faea`: валидное описание правки на 2000 знаков прежде растягивало
  очередь (RED: описание 1667.5 px). Теперь preview ограничен тремя строками,
  полный текст сохранён в DOM/title. GREEN проверяет высоту очереди <180 px
  и положение таблицы в viewport на 1366/1920; 20 unit + 6 browser PASS.
- `7f6f394`: создание сюжета и справочник рубрик используют общую типографику
  и 32 px кнопки; в создании тёплый footer и порядок «Отмена → Создать» справа.
  Это замена прежних правил stories.css, без параллельных overrides.
Живой frontend повторно проверен на обоих desktop-размерах. Профиль и смена
пароля просмотрены без ввода данных, создание сотрудника — по свежему
browser screenshot; дополнительных изменений им в этом этапе не потребовалось.
Focused rereview `c889443..7f6f394`: Approved, Critical/Important/Minor нет.
Свежий frontend: 568 passed / 59 files, build PASS. 12 SHA256 исходников
контейнера совпадают с рабочим деревом; synthetic backend/DB сохранены.

При полном browser-прогоне с 4 workers один раз не восстановился focus
после Escape в существующей проверке поиска редактора (154 passed, 1 failed,
2 BFCache skips). Шесть изолированных повторов на двух размерах прошли.
Editor-код не менялся; причина единичной нестабильности не доказана.
Исходный trace сохранён в output/ui-polish-2026-09-20/editor-focus-first-failure.
Повторный полный прогон с 2 workers фиксируется ниже, а не подменяет этот результат.

Повторный полный прогон с 2 workers: снова 154 passed / 1 failed / 2 skips,
та же проверка focus, теперь 1920. Проблема передана на отдельную диагностику
и детерминированное воспроизведение; это открытый gate, завершение пока не заявлено.
Оба failure traces сохранены. Все остальные browser-проверки нового дизайна прошли.


Дополнительная диагностика focus завершена коммитом `8b8599a`:
canonical поле при pending→grant сохраняет DOM identity, но краткий промежуток
с активным body при открытии поиска терял адрес возврата. Новый тест управляет
выдачей lease и воспроизводит этот промежуток (RED: inactive после Escape).
Теперь только для activeElement=body используется последнее подключённое поле;
открытие кнопкой возвращает focus кнопке. Ссылка сбрасывается при конфликте
и смене сценария. Изменений текста, autosave, прав или дизайна редактора нет.
Пробный переход с requestAnimationFrame на layout effect не помог и полностью
удалён. GREEN: исходный и новый сценарии — 32 browser-повтора на двух размерах,
компонентный поиск 1/1. Общие прогоны и независимое ревью фиксируются ниже.


Итоговый checkpoint 20 сентября закрыт. На коде `8b8599a`:
- frontend **568 passed / 59 files**, TypeScript/Vite build PASS;
- полный browser **157 passed / 2 BFCache skips**, 1.2 min, 0 failed;
- независимое focused review `7f6f394..8b8599a` — Approved,
  Critical/Important/Minor нет;
- 13 SHA256 исходников запущенного frontend совпадают с рабочим деревом;
- живой поиск открывается и закрывается с возвратом focus кнопке, console errors нет;
- backend предыдущего полного прогона: 1072 passed / 4 PostgreSQL-only skips;
  после него backend не менялся; docs checks после уточнений: 6 passed.

Старые карточные правила уведомлений и прежняя раскладка полей правок заменены
в тех же stylesheets; новая параллельная дизайн-система не создавалась.
Сохранённые PNG Paper использованы как утверждённый вход, новых вызовов **0**.
Остаточные ограничения: BFCache в headless и PostgreSQL-only skips отмечены выше;
унаследованные deprecation warnings и npm advisories не относятся к UI-этапу.
Все найденные замечания UI/review закрыты. Данные и право редактирования текста
не меняются; дополнительная правка сценария касается только возврата focus.
Работа локальная: main `d7a0300` не изменён, внешних push/PR/merge/deploy нет.
Точные логи, скриншоты и сохранённые failure traces лежат в
`output/ui-polish-2026-09-20/`; их история не заменена успешным итоговым прогоном.


### Цвета кнопок добавления блоков — уточнение пользователя

В `frontend/src/styles/scenario.css` кнопки «+ Подводка / ЗК / ЗК+гео / Лайф / СНХ»
используют общую переменную цвета с соответствующим select колонки «Блок».
Добавлена мягкая 9% заливка и 25% цветная рамка; прежние одинаковые синие
цвета кнопок заменены. Размеры, обработчики, шрифты текста и содержание
сценария не менялись. Изменение выполнено в текущей UI-ветке по прямой правке
пользователя к принятому макету; Paper не использовался.
Проверено на живом preview при 1133×800: цвет текста совпадает с типом блока,
кнопки различимы, console errors нет. CSS контейнера совпадает по SHA256.
Полный frontend 568/568, build PASS; browser 157 passed / 2 BFCache skips.


Уточнение палитры по следующему комментарию пользователя: близкие оттенки
ЗК+гео и Лайф разведены. ЗК+гео теперь фиолетовый `#6941c6`, Лайф сохраняет
янтарный `#b54708`. Общая переменная синхронно окрашивает кнопку и тип блока.
Каталог UI_SYSTEM_RU.md обновлён; исходный красно-оранжевый цвет заменён.
Живой preview проверен при 1133×800, фактические RGB совпадают с CSS,
console errors нет. CSS обновлён через Vite HMR без перезагрузки сценария,
образ frontend также пересобран. Полный frontend 568 passed, build PASS,
browser 157 passed / 2 BFCache skips. Paper: 0 новых вызовов.


### Материалы — ссылки и копирование без локального компонента

Пользователь отклонил установку обработчика папок и утвердил простой вариант
в MATERIAL_LINKS_PLAN_RU.md. `c547f75` — чистый helper адресов и 24 теста;
`acfefe7` — UI копирования, 6 component-тестов и browser-сценарий на двух размерах.
HTTP(S) открывается отдельно; UNC/SMB копируется для Windows/Linux, прочий путь
копируется без выдуманных преобразований. В том числе поддержаны пробелы,
кириллица, проценты, trailing separator и malformed UTF без падения страницы.
Clipboard API имеет fallback для HTTP-интранета; при отказе показано выделенное
поле ручного копирования. Недопустимые схемы ссылками не становятся.

RED: 5 из 6 UI-тестов падали на прежнем span; helper RED отсутствующего модуля
и отдельные 4 RED для крайних случаев. GREEN: 598 frontend / 61 files,
TypeScript/Vite PASS, 159 browser / 2 BFCache skips. Первое browser-падение
нового теста было ошибкой UTF-8 заголовка в mock HTML; исправлена только fixture.
В живом приложении проверено успешное «Ссылка скопирована» на synthetic URL;
проверены screenshots 1366/1920 и совпадение SHA256 четырёх runtime исходников.
Backend/API/сохранённые материалы не изменялись; Paper/установки/внешние действия — 0.
Результаты и screenshots: output/material-links-2026-09-20/.

Финальное независимое ревью выявило потерю клавиатурного фокуса во время
асинхронного копирования. Исправлено в `4d41ed1`: aria-disabled сохраняет focus,
синхронный inFlight блокирует повторный запуск. Browser-тест задерживает Clipboard
Promise и проверяет focus и подтверждение только после завершения; RED → GREEN.
Повторное ревью: Approved, открытых замечаний нет. Финальный прогон на `4d41ed1`:
598 frontend tests / 61 files, TypeScript/Vite PASS, 159 browser passed /
2 BFCache skips. Runtime SHA256 четырёх исходников повторно совпал после
пересборки preview. План завершён; реализация остаётся в локальной UI-ветке.

### Автор в исполнителях и вычитка рядом со статусом — 20 сентября

Пользователь утвердил file-level план в чате: убрать «Изменить» из шапок,
перенести автора в Исполнители, редакционные MUI-действия разместить рядом
со статусами в сценарии. Отдельно подтверждено сохранение права вычитки
для назначенного корректора, начальника и шефа. Backend policy не меняется.
Работа ведётся в codex/ui-polish-2026-09-20 от b1ee173; Paper не используется.
TDD для сценария: 5 RED / 9 pass → 14 GREEN; browser 2/2 на 1366/1920:
отметка отправляется после сохранения и освобождения lease, editor не remount.
Автор перенесён в `841bea9`, редакционные действия — в `f99c833`. При полном
unit-прогоне выявлены 4 failure в Истории из-за оставшегося использования старого
авторского контрола; оно удалено в том же checkpoint. Full browser выявил
2 устаревшие проверки позиции кнопки в шапке; в `b41fb19` они перенесены
к статусам, сохранив pending/error assertions; focused GREEN 2/2.
Первый backend-запуск из корня репозитория не нашёл migrations; правильный cwd
backend дал 22/22 targeted+docs. Это ошибка команды, код backend не менялся.
Независимое ревью нашло P2: после failed PATCH на другого автора, возврата
к исходному и PATCH ack / GET failure терялась цель повтора. В `4fef27c`
очистка цели отложена до успешного завершения без pending/ack/error.
Regression RED 1/6 → GREEN 34/34; повторное ревью Approved, замечаний нет.
Backend policy не менялся; полный backend 1072 passed / 4 skips.
Финальный параллельный browser-прогон выявил отдельную существующую
нестабильность ArchivePage focus (160 passed / 1 failed / 2 skips): rAF мог
выполниться до закрытия Dialog. Трассировка сохранена. Целевой повтор 2/2;
затем опасный порядок детерминированно воспроизведён unit-тестом (RED 1/9).
В `29b7509` восстановление перенесено в эффект после commit; guards от
перехвата пользовательского фокуса и устаревшей операции сохранены.
GREEN 10/10, независимое ревью Approved. Права/семантика удаления не менялись.
Итог на `29b7509`: 602 frontend tests / 61 files, TypeScript/Vite PASS,
161 browser passed / 2 BFCache skips. Полный backend — 1072 passed / 4 skips;
проверка документации 6/6. Все найденные замечания закрыты.
Живой preview обновлён, 11 runtime-файлов совпадают по SHA256 с проверенным
кодом. В браузере подтверждены автор первым в исполнителях и вычитка у
шефа рядом с корректурой; console errors нет. Скриншоты 1366/1920 просмотрены.
Удалены старый диалог автора, три шапочных control, portal workflow и прежние
CSS overrides. Сохранены шрифты/таблица/lease/autosave и действующие права.
Артефакты, RED/GREEN логи и исходная архивная failure trace сохранены в
output/author-workflow-2026-09-20/. Работа завершена локально, main не изменён,
Paper/CodeRabbit/push/PR/merge/внешний deploy не использовались.

### Действия у производственных этапов и общий диалог озвучки — 20–21 сентября

Пользователь разрешил реализацию в новой задаче от обязательной базы `786322b`
ветки `codex/ui-polish-2026-09-20`. Чистый worktree `3b4f/NewscastNavigator`
закреплён за локальной веткой `codex/production-stage-actions-2026-09-20`.
Read-only проверены AGENTS.md, SPEC, EVAL_RUBRIC, IMPLEMENTATION_PLAN,
GOAL_PROMPTS и UI_SYSTEM. Исходный план правки: StoryProductionPage,
ProductionStages/ProductionActions, production.css, component/browser-тесты
и документация. Прямые уточнения пользователя 21 сентября расширили его:
допуск «Готово к титрам» относится к «Титрам», а возврат озвучки использует
общий диалог правок. Для них также изменены два server action label,
CorrectionPackageDialog/CorrectionPartFields и тест единства подписи в API.

Изменено:

- Действия распределяются по смысловому этапу, а не по позиции в массиве.
  Все десять production-команд проверены на единственное размещение.
  Главное действие сохраняет серверное emphasis и маркер первичности;
  эфир/архив/возврат в работу и действие без подходящего этапа остаются в шапке.
- `video_approve_for_titles` находится в «Титрах». Подпись «Готово к титрам»
  одинакова в карточке и очереди внимания. Command code, endpoint, payload,
  роли и gates сохраняются; «Ролик готов»/возврат ролика остаются в «Монтаже».
- Возврат озвучки открывает общий CorrectionPackageDialog с фиксированной
  областью «Озвучка», исполнителем и описанием. Сохраняется прежний
  `/production/voiceover/not-ready` с description/assignee_user_id, а не
  подменяется общей командой создания пакета. Options берутся из production,
  поэтому недоступность загрузки остальных правок не лишает озвучку исполнителей.
- Белая панель этапов, общий MUI ActionButton и существующие токены;
  кнопки стоят рядом со статусом, длинные подписи переносятся.
- Удалены разделение `slice(0, 2)`/`slice(2)`, встроенная native-форма
  озвучки, её отдельное состояние и CSS. Общие подтверждения, single-flight,
  локальный текст при ошибке, focus trap/отмена и GET-only retry сохранены.
  SPEC, EVAL_RUBRIC и UI_SYSTEM приведены к уточнённому интерфейсу.

TDD: исходный component RED 12 failed / 21 passed → 33 passed.
Допуск в «Титрах»: отдельный RED на размещение и RED backend на новую подпись
→ GREEN. Диалог озвучки: 2 RED → 43/43 targeted (production + corrections).
Тесты проверяют точный payload, подтверждение/отмену, сохранение текста и
исполнителя при ошибке, общий pending, запрет дублей и только GET после ack.
В browser также воспроизведено смещение соседней кнопки на 156 px при старой
встроенной форме; общий диалог сохраняет геометрию панели.

Финальные проверки: полный frontend 615 passed / 61 files, TypeScript/Vite
PASS; полный backend 1072 passed / 4 PostgreSQL-only skips (279.27 s),
targeted backend после переименования 37 passed. Полный browser после всех
уточнений: 161 passed / 2 BFCache skips, включая 1366/1920 и wide-layout.
При переносе browser-теста старый точный селектор «Ответственный» не находил
поле общего диалога: исправлен scope локатора, повторный полный прогон PASS.
Снимки панели и заполненного диалога на 1366/1920 осмотрены; нет наложений,
горизонтальной прокрутки, Vite overlay или неожиданных console/page errors.
Browser-путь подтверждает исходный payload возврата озвучки, создание пакета,
переход «Готово к титрам» → «Начать титры», приёмку и возврат на правки.
Отдельный повтор сохраняет заполненный диалог с enabled submit без анимаций.

Самостоятельное финальное ревью Codex: незакрытых замечаний по изменению нет.
Оставшиеся ограничения проверки — PostgreSQL-only блокировки/индексы и BFCache,
а также отсутствие обновления существующего демонстрационного runtime. Выпуск,
внешний deploy и clean-deploy rehearsal в этот локальный UI-checkpoint не входят.

Проверочная среда: Playwright/Chromium, отдельный `127.0.0.1:5183`,
1366×768 и 1920×1080, плюс существующий wide-layout 2560. Browser plugin
в сессии отсутствует; используется установленный Playwright проекта.
Browser API синтетический; backend pytest использует отдельную временную SQLite.
Compose проверяется `docker compose --env-file .env.example -f compose.yaml config --quiet`.
Логи, RED/GREEN, traces и screenshots: `/private/tmp/newscast-production-actions/`.
Существующий preview `127.0.0.1:5173`, его backend и демобаза не изменялись.
Paper, CodeRabbit, push, PR, merge и внешний deploy не выполнялись.

### Возврат этапов на правки и пустой список пакетов — 21 сентября

После просмотра страницы пользователь утвердил единый контурный стиль трёх
кнопок возврата, подпись «Вернуть озвучку на правки» и удаление пустой карточки
«Правки». Код изменён в том же worktree и отдельной ветке.

- Возвраты озвучки, ролика и титров всегда выглядят как вторичные действия.
  Если доступна приёмка титров, она остаётся единственной выделенной кнопкой:
  серверная политика выбирает главным первое действие, не возвращающее этап
  на правки. Коды команд, адреса и payload не менялись.
- При отсутствии пакетов пустая карточка и сообщение «Правок пока нет» скрыты.
  Отдельная кнопка «Добавить правки» сохранена для общего пакета. Когда пакеты
  появляются, раздел называется «Пакеты правок» и показывает их полностью.
- Новый сценарий закреплён backend-, component- и browser-тестами. Сначала
  получены RED для подписи, приоритета и пустой карточки, затем GREEN.

Проверки: frontend 616/616, TypeScript/Vite PASS, compose config PASS,
browser 163 passed / 2 BFCache skips на 1366/1920 и wide-layout 2560.
Снимки нового состояния на 1366/1920 просмотрены; наложений нет. Полный
backend-прогон: 1074 passed / 4 PostgreSQL-only skips (326.88 s). Полный browser-тест создал
временные `output/`-снимки; они удалены после проверки.

Текущий `127.0.0.1:5183` показывает обновлённый frontend, но получает данные
от прежнего демонстрационного backend. Поэтому на живом сюжете подпись озвучки
до обновления backend остаётся прежней; синтетический browser-тест проверяет
новую подпись и форму. Демобаза и удалённый runtime не менялись.

### Единое оформление управления рубриками — 21 сентября

По отмеченному пользователем окну исправлено смешение прежних native-контролов
и общей MUI-темы. `RubricManagementDialog` теперь использует стандартные
Dialog, DialogTitle, DialogContent, TextField, Button, IconButton, Chip и Alert.
Собственные стили отвечают только за сетку и адаптивную раскладку; старые стили
полей, кнопок, фона и status-chip удалены. Команды API, сохранение локальных
черновиков при canonical refetch, блокировка повторных действий и возврат фокуса
сохранены.

TDD: component-тест сначала упал на прежнем `rubric-management-dialog` вместо
`MuiDialog-paper`, затем 2/2 прошли. Browser-тест закрепляет общую тему полей,
основной кнопки, вторичных действий и статуса, а также прежние POST/PATCH payload.
Снимки 1366/1920 и живое окно с восемью рубриками просмотрены: наложений нет,
длинный список прокручивается внутри окна, узкая панель складывает строку
вертикально.

Финальные проверки: frontend 616 passed / 61 files, TypeScript/Vite PASS,
compose config PASS; browser 163 passed / 2 BFCache skips, включая 1366/1920
и wide-layout 2560. Backend не менялся и повторно не запускался; его последний
полный результат этого checkpoint-chain — 1074 passed / 4 PostgreSQL-only skips.
Preview `127.0.0.1:5183` обновлён через Vite HMR и оставлен открытым на окне
рубрик. Демобаза, backend, push, PR, merge и deploy не выполнялись.

### Сворачиваемые инструменты сценария в режиме чтения — 21 сентября

По отмеченной пользователем области панель поиска, добавления блоков,
оформления и DOCX-экспорта в режиме чтения теперь по умолчанию скрыта. В строке
режима появилась компактная MUI-кнопка «Показать инструменты» / «Скрыть
инструменты» с `aria-expanded` и `aria-controls`. При переходе в редактирование
панель всегда открывается и отдельный переключатель скрытия исчезает; после
выхода из редактирования или перехода к другому сюжету режим чтения снова
начинается со свернутой панели. Cmd/Ctrl+F сам раскрывает панель и открывает
поиск, поэтому клавиатурный сценарий сохранён.

TDD: два новых component-теста сначала получили RED на отсутствующей кнопке и
видимых по умолчанию инструментах, затем GREEN. Browser-тест проверяет полный
путь: начальное скрытие, ручное раскрытие и повторное скрытие, Cmd/Ctrl+F и
постоянно открытую панель в редактировании. Старые component- и browser-сценарии
явно раскрывают панель или входят в редактирование там, где это требуется их
предусловиями.

Финальные проверки: frontend 618 passed / 61 files, TypeScript/Vite PASS,
compose config PASS; browser 165 passed / 2 существующих BFCache skips,
включая 1366/1920 и wide-layout 2560. Живой preview `127.0.0.1:5183` проверен
на сюжете 21: оба состояния отображаются без overlay, ошибок и предупреждений
в консоли. Preview оставлен открытым в свернутом состоянии по умолчанию.
Backend не менялся и повторно не запускался; демобаза, push, PR, merge и deploy
не выполнялись.

### Сквозная унификация интерфейса — 21 сентября

По прямому запросу пользователя выполнена полная ревизия frontend-поверхностей.
Список и создание сюжетов, фильтры, приоритеты, рубрики, уведомления, release
notes, производство, пакеты правок, внешнее согласование, материалы, история,
сотрудники и окружающие контролы сценария переведены на общую MUI-тему и
семантические токены. Удалён неиспользуемый `ActionButton`. Внутренние поля
табличного редактора, Tiptap, drag-and-drop и resize сохранены как явно
ограниченное исключение рабочего контракта сценария.

`uiSystemContract.test.ts` рекурсивно запрещает старые варианты кнопок,
собственные диалоги, обычные native-контролы вне разрешённого ядра сценария,
локальные литералы цветов, имитацию компонентов в `base.css` и runtime-вызовы
`confirm/alert/prompt`. Все подтверждения действий теперь используют общий MUI
`ConfirmationDialog`. Диалоги правок и внешнего результата сохраняют черновик
при клике на фон; длинная ошибка остаётся видимой рядом с закреплёнными
действиями. Уведомления и модальные окна возвращают фокус после Escape/отмены.

Асинхронное подтверждение выхода из редактора сохраняет нативную историю:
отмена Back возвращает прежнюю запись, подтверждение не создаёт дубликат, а
Forward при открытом окне отменяет устаревший callback. Сначала получены RED на
дублирование истории и Back → Forward, затем оба пути прошли unit- и настоящую
Chromium-проверку. BFCache-probe дополнительно ждёт готовую служебную страницу,
чтобы Vite reload не перехватывал `goBack`.

Финальные проверки после всех исправлений: frontend `625 passed / 62 files`,
TypeScript/Vite PASS (`1089 modules`), Compose config PASS; полный browser
`167 passed / 2` ожидаемых BFCache skips на 1366×768, 1920×1080 и wide-layout
2560×1440. Изолированный BFCache-probe после стабилизации прошёл шесть
параллельных повторов без падений (шесть корректных skips среды). Последний
полный backend этого frontend-only checkpoint-chain: `1074 passed / 4`
PostgreSQL-only skips.

Живой `127.0.0.1:5183` осмотрен на сценарии, производстве, списке сюжетов и в
окне рубрик; ошибок и предупреждений браузера нет. Оставлен один Vite-процесс
на `5183`, временный `5199` завершён Playwright. Cursor и Gemini не использовались:
их инструменты не подключены, а локального набора и независимых Codex-ревью было
достаточно. Демобаза, backend, push, PR, merge и deploy не выполнялись.

### Регрессионная доводка единого интерфейса — 21 сентября

После сквозной замены контролов выполнен отдельный визуальный проход по всему
frontend и исправлены побочные изменения геометрии. В сценарии восстановлены
размеры полей названия, рубрики и хронометража; семантические цвета кнопок
«Подводка», «ЗК», «ЗК+гео», «Лайф» и «СНХ»; круглая форма палитры и оформление
выбора шрифта. Синяя рамка при вводе в Tiptap заменена нейтральной внутренней
линией. На компактной ширине заголовок и toolbar остаются внутри страницы, а
кнопка удаления выбранной строки больше не сжимается и не перекрывает подсказку.

В остальных поверхностях восстановлены две колонки функций сотрудника,
геометрия значка, счётчика и подписи уведомлений и полные подписи приоритетов.
После дополнительного замечания пользователя реестр сюжетов проверен отдельно:
фильтры уменьшены до 36 px и лишены тяжёлой тени, их подписи центрированы с
отклонением 0,34 px; приоритеты уменьшены до 100×26 px, текст и стрелка имеют
нулевое смещение от центра, а первая колонка сжата до 122 px. На 900 px при
переносе действий больше не остаётся отдельный вертикальный разделитель.

Для каждого обнаруженного дефекта добавлена browser-регрессия. Проверка
приоритета отдельно прошла цикл RED на высоте фильтров и RED на смещении стрелки
2,156 px, затем GREEN. Независимый browser-review на 1366×768, 900×768 и
1920×1080 не нашёл оставшихся замечаний по реестру. Полный browser-прогон:
169 passed / 2 ожидаемых BFCache skips из 171; wide-layout 2560×1440 включён.
Последний полный frontend unit-прогон этого checkpoint-chain: 625 passed /
62 files. Backend не менялся; его последний полный результат — 1074 passed /
4 PostgreSQL-only skips. Демобаза, push, PR, merge и deploy не выполнялись.
