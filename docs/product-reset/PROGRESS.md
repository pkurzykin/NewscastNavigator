...
- общий PostgreSQL-aware SQL-order guard fail-closed требует отдельные single-target `FOR UPDATE` statements со строгими индексами Story < Scenario < Workflow < Production < Session, не считает scalar-subquery markers отдельными locks и запрещает post-session relock любой из четырёх aggregate tables; первичный meta-contract: `2 failed / 2 passed` до hardening, затем `5 passed`; повторный RED на collapsed multi-table statement: `1 failed`, после structural correction строгий meta-contract: `14 passed`, реальные save/workflow/active GET/history restore traces: `4 passed`, archive+history scope: `39 passed`;
