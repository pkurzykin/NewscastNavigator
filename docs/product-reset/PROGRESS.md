...
- общий PostgreSQL-aware SQL-order guard fail-closed требует Story → Scenario → Workflow → Production до первой Session lock и запрещает post-session relock любой из четырёх aggregate tables, не смешивая ordinary SELECT с `FOR UPDATE`; meta-contract: `2 failed / 2 passed` до hardening, затем `5 passed`, реальные save/workflow/active GET/history restore traces: `4 passed`, archive+history scope: `30 passed`;
