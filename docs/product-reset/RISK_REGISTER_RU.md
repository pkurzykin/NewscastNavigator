...
CP2 фиксирует single migration, actual synthetic seed, clean schema и единственный временный bridge; runner-owned command evidence привязан к clean source `60c8f6721bcd3053c11fa2eb2316c8d8e94616fa`. `failed_gates` теперь содержит CP3–CP7 и `external_demo`, поэтому final verify остаётся красным. Browser runner CP2 также не записан как успешная проверка.
