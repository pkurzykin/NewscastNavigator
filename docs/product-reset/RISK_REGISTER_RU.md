...
| R05 | Ложный зелёный eval | Снижен: CP1 привязан к `ee8efc5...`, CP2 — к `60c8f6721bcd3053c11fa2eb2316c8d8e94616fa`; CP3 source/template требует точную evidence и остаётся unbound/failed до runner-owned boundary | evaluator проверяет schema, exact commands/counts/hashes, Git ancestry/tree, bridge absence, per-command HEAD/source cleanliness и отдельные checkpoint binding | после независимого review выполнить CP3 runner на clean source commit и записать отдельный binding commit |
...
Commit 3.5 source/template фиксирует exact CP3 evidence schema, канонические runner-owned команды и immutable Git binding contract. Source template не объявляет CP3 пройденным: `evaluated_commit` не связан, command results пусты, CP3 остаётся в `failed_gates`. После independent review нужен отдельный runner-owned boundary-run и binding commit.
