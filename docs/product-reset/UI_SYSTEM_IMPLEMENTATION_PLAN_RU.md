...
- По прямому уточнению 15 сентября таблица использует вариант `2026-09-14-local-editor-style`, ранее оценённый «3 с минусом». Прежнее откладывание пробы отменено; все функции и настройки шрифтов сохраняются.
...
- [x] По уточнению пользователя 20 сентября автор выбирается первой строкой в «Производство → Исполнители»: server options, без «Без автора», явный выбор сохраняется сразу, ошибки сохраняют выбор; stale/ack guards и общая координация команд. Старый диалог и кнопки «Изменить» удалены.
...
- [x] Применить общий дизайн к истории/diff, уведомлениям, формам; не добавлять новые продуктовые функции. Первоначальное исключение таблицы отменено прямым уточнением 15 сентября: выбран вариант 14 сентября, см. дополнение ниже.
...
## Визуальная доводка 15 сентября — подтверждена пользователем

Запрос: «давай доводи вообще всё, и таблицу редактор тоже, с тем как мы ее приняли».
Работа продолжает утверждённые макеты 6/13 сентября и не вводит новый продуктовый режим.

- `frontend/src/styles/base.css`: базовые native controls не перекрывают компонентные размеры и цвета.
- `ScenarioEditor.tsx`, `ScenarioHistoryControls.tsx`, `StoryScenarioPage.tsx`, `styles/scenario.css`: компактные workflow/access/toolbar, один заголовок, действия в шапке; неизменные пять столбцов, синяя шапка и функции.
- `styles/{stories,production,history,admin,notifications,external-approval,corrections}.css` и соответствующие компоненты: плотность принятых экранов, фильтры/список, диалоги и пользовательские подписи.
- Регрессии: browser geometry/цвет/режим доступа в `editor-typography.spec.ts`; существующие сценарные, архивные, notification и component-тесты.
- Проверки: полный frontend/build/browser, визуальная сверка снимков и smoke локального стенда, независимое ревью Codex, документация и локальный commit.
- Эталонные Paper exports неизменны. Новых вызовов Paper не требуется.


## Уточнение 15 сентября — trial 14 сентября и полная очистка frontend

Пользователь прямо выбрал ранее отложенный вариант «3 с минусом» и поручил
перепроверить весь frontend. Это заменяет прежнее исключение trial из внедрения.
Остальные продуктовые/шрифтовые/lease/CaptionPanels требования сохраняются.

- `ScenarioEditor.tsx`, `ScenarioRow.tsx`, `ScenarioHistoryControls.tsx`, новый `ScenarioIcon.tsx`, `styles/scenario.css`: перенести визуальную часть local-editor-style 14 сентября, заменить dropdown основного шрифта двумя кнопками без нового механизма сохранения, очистить дубли прежних CSS rules.
- Все 11 `frontend/src/styles/*.css`, `shared/ui/theme.ts` и компоненты остальных экранов: инвентаризация дублей/оставшихся legacy defaults/inline оформления, нейтральные native controls, единые semantic tokens и устранение противоречащих overrides.
- Существующие font/autosave/DOCX/browser тесты адаптировать к новому элементу управления, не ослабляя проверку snapshot/font/selection/lease. Новая browser-регрессия проверяет визуальные признаки trial.
- Финальные gates: полный frontend/build/browser, осмотр снимков редактора/Истории/остальных экранов, CSS inventory и независимое ревью, актуальная документация и локальные commits.
- По последнему указанию пользователя безопасный порог остановки основной квоты — 3%.
