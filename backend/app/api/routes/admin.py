...
from app.services.user_deletion import find_user_deletion_blockers
...
@router.delete("/users/{user_id}", response_model=CommandAck)
def delete_user(
    user_id: int,
    db: Session = Depends(get_db),
    actor: User = Depends(require_chief),
) -> CommandAck:
